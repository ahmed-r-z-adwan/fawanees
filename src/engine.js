...
(function (root) {
  ...
  class Engine {
    constructor(geo, rules, weights) {
      ...
      this.ttBits = 20; this.ttSize = 1 << this.ttBits; this.ttMask = this.ttSize - 1;
    ...
    terminalValue(board, p, sc) {
      const [a, b] = score(this.geo, board, sc, this.rules.komi);
      const d = p === 1 ? a - b : b - a;
    ...
    negamax(board, side, depth, alpha, beta, ply, prevPass, h1, h2) {
      ...
      computeLight(this.geo, board, sc);
      ...
      const nb = this.boards[ply + 1];
      for (let i = 0; i < n; i++) {
        const m = moves[i];
        nb.set(board);
        let v;
        if (m < 0) v = prevPass ? this.terminalValue(nb, side, this.scs[ply + 1]) : -this.negamax(nb, 3 - side, depth - 1, -beta, -alpha, ply + 1, true, h1, h2);
        else { applyMove(this.geo, nb, side, m, this.rules, this.scs[ply + 1], false); v = -this.negamax(nb, 3 - side, depth - 1, -beta, -alpha, ply + 1, false, side === 1 ? h1 - 1 : h1, side === 2 ? h2 - 1 : h2); }
        if (this.stop) return 0;
